...
from tensorflow_model_analysis.extractors import input_extractor_v1
...
class TFMAV2BenchmarkBase(benchmark_base.BenchmarkBase):
  ...
  def benchmarkInputExtractorManualActuation(self):
    ...
    start = time.time()
    for elem in records:
      extracts.append(input_extractor_v1._ParseExample(elem, self._eval_config))  # pylint: disable=protected-access
    end = time.time()
    delta = end - start
    self.report_benchmark(
